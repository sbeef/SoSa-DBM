    ...
    (begin
      ...
      (define input-tabs
        (new tab-panel%
             ...
             [parent entry-panel]
             [callback (lambda (tab-panel event) (show-tab cur-tab tab-panel entry-panel))]))
      ;the tab pannels
      (define tab-pannels (draw-tabs cur-tab-list 0 input-tabs))
      (clear-panel input-tabs)
      (load-tab cur-tab input-tabs)
      ;(send frame show #t)
      )))
...
(define load-tab
  (lambda (tab-id panel)
    (letrec ([tab-fields (set-and-draw-fields tab-id panel)]
...
; shows the right tab
(define show-tab
  (lambda (cur-tab tab-panel panel)
    (begin
      (save-tab cur-tab)
      (clear-panel tab-panel)
      (load-tab (send tab-panel get-selection) tab-panel)
